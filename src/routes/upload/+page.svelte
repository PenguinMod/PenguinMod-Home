<script>
  ...
  $: filteredProjects =
    ((projectRemixSearchQuery ?? '') !== ''
      ? canRemix.filter(
          (project) =>
            [project.title, project.instructions, project.notes, project.id].join('')
              .toLowerCase()
...
<div class="main">
  ...
  {#if loadingExternal}
    <div class="external-loading">
      ...
      <p style="text-align: center;">
        {@html TranslationHandler.text('project.importing', currentLang)}
      </p>
    </div>
  {/if}
        ...
        <div class="card-projects">
          ...
          {#if !lastProjectPage}
            ...
            <Button
              ...
              on:click={() => incrementPageAndAddToMenu(projectPageType)}
            />
          {/if}
        </div>
    ...
    <div class="front-card-page">
      <div class="card-page">
        ...
        <div class="search-input">
          ...
          >
            <SearchSVG width="22px" height="22px" color="#ffffff" scale="2px" />
          </Button>
        </div>
        <div class="card-projects">
  ...
  <div class="section-info">
    <h1>
      <LocalizedText text="Upload" key="uploading.title" lang={currentLang} />
    </h1>
  </div>
          ...
          {#key recommendedTagUpdate}
            {#each recommendedTagList as recommendedTag}
              <button
                ...
                on:click={() => clickOnRecommendedTag(recommendedTag)}
              >
                + #{recommendedTag}
              </button>
            {/each}
        ...
        {#if loggedIn && projectData}
          <div>
            ...
            <div style="display:flex;flex-direction:row">
              ...
              {/if}
              {#if !remixedInURL}
                <Button color="remix" icon="remix.svg" on:click={openRemixMenu}>
                  <LocalizedText
                    text="Remix"
