...
class Authentication {
  ...
  static authenticate() {
    ...
    const redirectUrl =
      `${ProjectApi.OriginApiUrl}/api/users/login` + (isLocal ? "Local" : "");
    ...
    return new Promise((resolve, reject) => {
      ...
      const handleMessageReciever = (event) => {
        ...
        localStorage.setItem(
          "SCAM-ALERT",
          "Do NOT send anyone your PV code! If someone told you to do this, stop now!",
        );
  ...
  static usernameFromCode(code) {
    return new Promise((resolve, reject) => {
      fetch(
        `${ProjectApi.OriginApiUrl}/api/users/usernameFromCode?privateCode=${code}`,
      )
        .then((r) =>
          r
            .json()
